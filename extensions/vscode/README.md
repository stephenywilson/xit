...
鼠标悬停在状态栏上可查看更多信息，包括历史累计节省量和最近的本地 XiT 记录来源。

**边界说明：** 吸T神功**不会读取** Claude、Codex、Gemini、Cursor 的聊天内容，也不会读取私有 Webview 内部状态。状态栏不会显示“已连接 Claude/Codex/Gemini”这类文案；如有工具来源信息，也只会在 tooltip 或 Dashboard 中以“最近记录：Claude”这类历史记录方式展示。
