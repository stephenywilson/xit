...
| **Claude Code** | 🔄 适配中 | 后续做 hook 与路由命中率验证 |
| **Codex** | 📋 规划中 | 后续适配 |
| **Cursor** | 📋 规划中 | 后续适配 |
...
![Kimi 实战状态栏](docs/assets/kimi-toolbar.png)

Kimi CLI 是 XiT 第一套已跑通的实战适配，用来证明 rules、hook、turn lifecycle、中文状态栏这条链路可行。
...
可选中文状态栏（修改本地 Kimi package，可回滚）：

```bash
xit kimi status-patch install --yes --accept-risk
xit kimi status-patch uninstall --yes   # 回滚
```

状态栏示例：`吸T神功 · 准备就绪` → `正在吸T中` → `本次吸T 1次 · 省 9k Token`

详细文档见 [docs/kimi.md](docs/kimi.md)。
