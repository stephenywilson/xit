...
func TestClassifyPolicy(t *testing.T) {
	tests := []struct {
		...
		{"yarn test", []string{"yarn", "test"}, "should_compress"},
		{"yarn install", []string{"yarn", "install"}, "needs_review"},
		{"kubectl logs", []string{"kubectl", "logs", "pod-1"}, "should_compress"},
		{"kubectl get", []string{"kubectl", "get", "pods"}, "should_passthrough"},
		{"ruff", []string{"ruff", "check", "."}, "should_compress"},
