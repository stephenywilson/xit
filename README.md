...
| **Claude Code** | 🔄 observe 适配中 | CLAUDE.md rules + project hook 已进入命中率验证 |
| **Codex** | 🔄 rules 适配中 | AGENTS.md rules 已建立，等待 dogfood 验证 |
| **Antigravity CLI** | ✅ rules 已跑通 | agy 1.0.3；headless dogfood 已验证，高噪音命令可按规则使用 xit auto |
| **Cursor** | 📋 规划中 | 后续适配 |
...
- Claude Code observe hook + CLAUDE.md rules（命中率验证中）
- 更多高噪音命令过滤器
